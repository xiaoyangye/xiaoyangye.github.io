...
#let font_cn = ("SimSun", "Noto Serif CJK SC", "Source Han Serif SC", "Songti SC", "STSong")
#let font_cn_bold = ("SimHei", "Noto Sans CJK SC", "Source Han Sans SC", "Heiti SC", "STHeiti")
...
#set page(
  ...
  footer-descent: 14mm,
  footer: none,
  fill: white, // Ensure white background for all pages
)
#set text(
  font: FONT_BODY,
  size: size_x4,
...
#set par(justify: true, first-line-indent: 2em, leading: 0.65em) // Slightly increased leading for better look
...
#let heading_numbering(..args) = {
  let nums = args.pos()
  let l = nums.len()
  if l == 1 { "第" + number_to_chinese(nums.last()) + "章" + h(1em) } else if l == 2 {
    str(nums.first()) + "." + str(nums.last()) + h(1em)
  } else if l == 3 { str(nums.at(0)) + "." + str(nums.at(1)) + "." + str(nums.last()) + h(1em) } else { none }
}
#set heading(numbering: heading_numbering)

#show heading.where(level: 1): it => {
  let titles = ("摘要", "目录", "参考文献", "致谢", "Abstract", "附录", "绪论", "结论")
